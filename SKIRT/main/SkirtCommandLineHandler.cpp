...
void SkirtCommandLineHandler::doSimulation(size_t index)
{
    ...
    try
    {
        ...
        {
            // in data parallel mode, if there is dust emission, the spectra must be calculated for all cells
            // (in other words, there is no support for dust libraries)
            PanDustSystem* pds = simulation->find<PanDustSystem>(false);
            if (pds && pds->hasDustEmission() && !pds->find<AllCellsDustLib>(false))
                throw FATALERROR("When using -d, the dust emission can only be calculated using an AllCellsDustLib");

            simulation->communicator()->setDataParallel(true);
        }
