    ...
    for (int i=0; i<NZrel; i++)
        for (int j=0; j<NlogC; j++)
            for (int k=0; k<Nlogp; k++)
            {
                ...
                string filename = FilePaths::resource("SED/Mappings/Mappings_")
...
Array MappingsSEDFamily::luminosities(double SFR, double Z, double logC, double pressure, double fPDR, double z) const
{
    ...
    double logp = log10(pressure/Constants::k()*1e-6);
    ...
    int i = NR::locateClip(_Zrelv,Zrel);
    double hZrel = (Zrel-_Zrelv[i])/(_Zrelv[i+1]-_Zrelv[i]);
    int j = NR::locateClip(_logCv,logC);
    double hlogC = (logC-_logCv[j])/(_logCv[j+1]-_logCv[j]);
    int k = NR::locateClip(_logpv,logp);
...
int MappingsSEDFamily::numParams() const
{
    return 5;
}
...
Array MappingsSEDFamily::luminositiesGeneric(const Array& params, int skipvals, double z) const
{
...
double MappingsSEDFamily::massGeneric(const Array& params, int skipvals) const
{
...
string MappingsSEDFamily::sourceName() const
{
    return "hii";
}
...
string MappingsSEDFamily::sourceDescription() const
{
    return "HII region";
}
