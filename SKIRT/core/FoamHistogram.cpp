...
void FoamHistogram::Fill(double xx, double wt)
{
    // fill histogram with single weighted event
    int ib = static_cast<int>( (xx-m_xmin)/(m_xmax-m_xmin) *m_Nb) +1;
...
double FoamHistogram::GetBinContent(int nb)
{
...
double FoamHistogram::GetBinError(int nb)
{
    // Get content of bin nb.    nb=1 for 1-st bin, underflow nb=0, overflow nb=Nb+1
