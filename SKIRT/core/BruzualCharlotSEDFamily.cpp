...
void BruzualCharlotSEDFamily::setupSelfBefore()
{
    ...
    for (int m=0; m<NZ; m++)
    {
        string bcfilename = FilePaths::resource("SED/BruzualCharlot/chabrier/bc2003_lr_"
                                                 + Zcodev[m] + "_chab_ssp.ised_ASCII");
        std::ifstream bcfile = System::ifstream(bcfilename);
...
int BruzualCharlotSEDFamily::numParams() const
{
    return 3;
}
...
Array BruzualCharlotSEDFamily::luminositiesGeneric(const Array& params, int skipvals, double z) const
{
...
double BruzualCharlotSEDFamily::massGeneric(const Array& params, int skipvals) const
{
...
string BruzualCharlotSEDFamily::sourceName() const
{
    return "star";
}
...
string BruzualCharlotSEDFamily::sourceDescription() const
{
    return "star";
}
