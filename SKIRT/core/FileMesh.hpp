...
/** FileMesh is a subclass of the MoveableMesh class. It represents a one-dimensional mesh over the
    ...
    in arbitrary order. The points may be given in arbitary units. If the lowest point is not zero,
    an extra zero mesh point is inserted. The mesh is scaled so that the last point in the file is
    mapped to unity. */
class FileMesh : public MoveableMesh
{
    ITEM_CONCRETE(FileMesh, MoveableMesh, "a mesh read from a file")

    PROPERTY_STRING(filename, "the name of the file with the mesh points")
