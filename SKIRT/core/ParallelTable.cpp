...
void ParallelTable::initialize(string name, WriteState writeOn, const ProcessAssigner* colAssigner,
                               const ProcessAssigner* rowAssigner, PeerToPeerCommunicator* comm)
{
    ...
    // Inform the user about the mode used and the size of the locally stored data
    _log->info(_name + " table is distributed; local size switches between "
               + std::to_string(_totalRows) + "x" + std::to_string(_colAssigner->assigned())
               + " (" + StringUtils::toMemSizeString(_totalRows*_colAssigner->assigned()*sizeof(double)) + ") and "
               + std::to_string(_rowAssigner->assigned()) + "x" + std::to_string(_totalCols)
               + " (" + StringUtils::toMemSizeString(_rowAssigner->assigned()*_totalCols*sizeof(double)) + ")");
...
void ParallelTable::initialize(string name, WriteState writeOn, size_t columns, size_t rows,
                               PeerToPeerCommunicator* comm)
{
    ...
    // Report the allocation mode and table size
    _log->info(_name + " table is not distributed; size is "
               + std::to_string(_totalRows) + "x" + std::to_string(_totalCols)
               + " (" + StringUtils::toMemSizeString(_totalRows*_totalCols*sizeof(double)) + ")");
...
double ParallelTable::operator()(size_t i, size_t j) const
{
...
void ParallelTable::sumAll()
{
    if (_writeOn == WriteState::COLUMN)
    {
        Array& arr = _columns.data();
        _comm->sumAll(arr);
    }
    else
    {
        Array& arr = _rows.data();
        _comm->sumAll(arr);
    }
}
