...
FoamVector*& FoamCell::operator[](int i)
{
...
void FoamCell::Fill(int Status, int Parent, int Daugh1, int Daugh2,
                    int* Vertices, FoamVector* Posi, FoamVector* Size)
{
    ...
    if(Vertices!=NULL)
    {
        if( (m_Verts != NULL) || (m_nVert != 0) )
            throw FATALERROR("m_Verts already allocated, m_nVert=" + std::to_string(m_nVert));
        m_nVert = m_nDim+1;
...
void FoamCell::GetHcub(FoamVector& Posi, FoamVector& Size)
{
...
void FoamCell::GetHSize(FoamVector& Size)
{
...
void FoamCell::GetXSimp(FoamVector& X, FoamVector& lambda, int kVert)
{
