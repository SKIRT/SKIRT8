...
{
    ITEM_ABSTRACT(MultiGrainDustMix, DustMix, "a multi-grain dust mix")

    PROPERTY_BOOL(writeSize, "output a data file with grain size information for the dust mix")
        ATTRIBUTE_DEFAULT_VALUE(writeSize, "true")
...
public:
    ...
    string grainCompositionName(int c) const;
...
private:
    // additional multi-grain properties initialized during setup
