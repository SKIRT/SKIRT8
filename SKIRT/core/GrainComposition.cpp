...
void GrainComposition::setupSelfAfter()
{
    ...
    log->info("   " + StringUtils::toString(_Nlambda) + " wavelengths"
              " from " + StringUtils::toString(units->owavelength(_lambdav[0])) + " " + units->uwavelength() +
              " to " + StringUtils::toString(units->owavelength(_lambdav[_Nlambda-1])) + " " + units->uwavelength() );
    log->info("   " + StringUtils::toString(_Na) + " grain sizes"
              " from " + StringUtils::toString(units->ograinsize(_av[0])) + " " + units->ugrainsize() +
              " to " + StringUtils::toString(units->ograinsize(_av[_Na-1])) + " " + units->ugrainsize() );
    log->info("   " + StringUtils::toString(_NT) + " temperatures"
              " from " + StringUtils::toString(units->otemperature(_Tv[0])) + " " + units->utemperature() +
            " to " + StringUtils::toString(units->otemperature(_Tv[_NT-1])) + " " + units->utemperature() );
}
...
double GrainComposition::specificEnthalpy(double T) const
{
    ...
    int t = NR::locateClip(_Tv, T);
    return NR::interpolateLogLog(T,  _Tv[t], _Tv[t+1],  _hv[t], _hv[t+1]);
}
...
void GrainComposition::loadOpticalGrid(bool resource, string name, bool reverse, bool skip1, bool skip2, bool skip3)
{
    // open the file
    string filename = resource ? FilePaths::resource(name) : find<FilePaths>()->input(name);
...
void GrainComposition::loadOpticalGrid(string resourceLambda, string resourceQ, string resourceG)
{
    // ------------ wavelengths file ------------
    {
        // open the file
        string filename = FilePaths::resource(resourceLambda);
        std::ifstream file = System::ifstream(filename);
        if (!file.is_open()) throw FATALERROR("Could not open the data file " + filename);
    ...
    {
        // open the file
        string filename = FilePaths::resource(resourceQ);
        std::ifstream file = System::ifstream(filename);
        if (!file.is_open()) throw FATALERROR("Could not open the data file " + filename);
    ...
    {
        // open the file
        string filename = FilePaths::resource(resourceG);
        std::ifstream file = System::ifstream(filename);
        if (!file.is_open()) throw FATALERROR("Could not open the data file " + filename);
...
void GrainComposition::loadEnthalpyGrid(bool resource, string name)
{
    // open the file
    string filename = resource ? FilePaths::resource(name) : find<FilePaths>()->input(name);
    std::ifstream file = System::ifstream(filename);
...
void GrainComposition::loadLogHeatCapacityGrid(string resourcename)
{
    // verify that the bulk density has been set before calling this function
    if (_rhobulk <= 0) throw FATALERROR("The bulk density must have a positive value");

    // open the file
    string filename = FilePaths::resource(resourcename);
    std::ifstream file = System::ifstream(filename);
    if (!file.is_open()) throw FATALERROR("Could not open the data file " + filename);
    ...
    Array logTv;
    double dT = NR::buildLinearGrid(logTv, logTinv[0], logTinv[Nin-1], _NT-1);
    Array logCv = NR::resample<NR::interpolateLinLin>(logTv, logTinv, logCinv);
...
void GrainComposition::calculateEnthalpyGrid(EnthalpyFunction efun)
{
    ...
    NR::buildLogGrid(_Tv, 1., 3000., _NT-1);
    _hv.resize(_NT);
...
void GrainComposition::loadPolarizedOpticalGrid(bool resource, string name)
{
    // open the file
    string filename = resource ? FilePaths::externalResource(name) : find<FilePaths>()->input(name);
    std::ifstream file = System::ifstream(filename);
    if (!file.is_open()) throw FATALERROR("Could not open the data file " + filename);
