...
public:
    ...
    SphericalAdaptiveMesh(AdaptiveMeshFile* meshfile, const vector<int>& fieldIndices, double rin, double rout);
    ...
    /** This function returns the value of the total density distribution integrated over the complete domain,
        ...
        D_h({\bf{r}}) \,\text{d}{\bf{r}} \approx \sum_{m=0}^{N_\text{cells}-1} D_h(m)\times V_m \f]. */
