...
void DustMix::setupSelfAfter()
{
    ...
    log->info("Dust mixture " + std::to_string(h));
    try
    {
        double kappaV = kappaext(Constants::lambdaV());
        log->info("Total extinction coefficient in the V band: kappaV = "
                  + StringUtils::toString(units->oopacity(kappaV)) + " " + units->uopacity());
    }
    ...
    {
        // Create a text file
        TextOutFile file(this, "ds_mix_" + std::to_string(h) + "_opti", "optical dust population properties");

        // Write the header
        file.addColumn("lambda (" + units->uwavelength() + ")");
        string section = units->usection() + " per hydrogen atom";
        for (int c=0; c<_Npop; c++) file.addColumn("sigma_ext[" + std::to_string(c) + "] (" + section + ")");
        for (int c=0; c<_Npop; c++) file.addColumn("sigma_abs[" + std::to_string(c) + "] (" + section + ")");
        for (int c=0; c<_Npop; c++) file.addColumn("sigma_sca[" + std::to_string(c) + "] (" + section + ")");
        for (int c=0; c<_Npop; c++) file.addColumn("asymmpar[" + std::to_string(c) + "]");

        // Write the body
        for (int ell=0; ell<_Nlambda; ell++)
        {
            vector<double> values;
            values.push_back( units->owavelength(_lambdagrid->lambda(ell)) );
            for (int c=0; c<_Npop; c++) values.push_back( units->osection(_sigmaabsvv[c][ell]+_sigmascavv[c][ell]) );
            for (int c=0; c<_Npop; c++) values.push_back( units->osection(_sigmaabsvv[c][ell]) );
            for (int c=0; c<_Npop; c++) values.push_back( units->osection(_sigmascavv[c][ell]) );
            for (int c=0; c<_Npop; c++) values.push_back( _asymmparvv[c][ell] );
    ...
    {
        // Create a text file
        TextOutFile file(this, "ds_mix_" + std::to_string(h) + "_mass", "dust population masses");

        // Write the header
        string bulkmass = units->ubulkmass() + " per hydrogen atom";
        file.writeLine("# total dust mass: " + StringUtils::toString(units->obulkmass(_mu), 'g') + ' ' + bulkmass);
        ...
        for (int c=0; c<_Npop; c++)
        {
            file.writeRow( vector<double>({static_cast<double>(c), units->obulkmass(_muv[c]), 100.*_muv[c]/_mu}) );
        }
    }
    ...
    {
        // Create a text file
        TextOutFile file(this, "ds_mix_" + std::to_string(h) + "_mean", "combined dust mix properties");
        ...
        for (int ell=0; ell<_Nlambda; ell++)
        {
            file.writeRow( vector<double>({ units->owavelength(_lambdagrid->lambda(ell)),
    ...
    if (_lambdagrid->isSampledRange())
    {
        ...
        const int NT = 1000;
        NR::buildPowerLawGrid(_Tv, 0., 5000., NT, 500.);
...
void DustMix::addPopulation(double mu, const Array& sigmaabsv, const Array& sigmascav, const Array& asymmparv)
{
    ...
    if (sigmaabsv.size()!=static_cast<size_t>(_Nlambda) ||
        sigmascav.size()!=static_cast<size_t>(_Nlambda) ||
        asymmparv.size()!=static_cast<size_t>(_Nlambda))
    {
                            ...
                            const Array& sigmaabsv, const Array& sigmascav, const Array& asymmparv)
{
    ...
    if (lambdagridv[0]<lambdav[0]*(1-eps) || lambdagridv[lambdagridv.size()-1]>lambdav[lambdav.size()-1]*(1+eps))
    {
        ...
        throw FATALERROR(
                    ...
                    StringUtils::toString(units->owavelength(lambdav[0])) + " and " +
                    StringUtils::toString(units->owavelength(lambdav[lambdav.size()-1])) + " " + units->uwavelength() );
    }

    // add the population using resampled properties
    addPopulation(mu, NR::resample<NR::interpolateLogLog>(lambdagridv,lambdav,sigmaabsv),
                      NR::resample<NR::interpolateLogLog>(lambdagridv,lambdav,sigmascav),
                  NR::resample<NR::interpolateLogLin>(lambdagridv,lambdav,asymmparv) );
}
...
double DustMix::kappaext(double lambda) const
{
    ...
    if (_lambdagrid->isSampledRange())
    {
        ...
        int ell = NR::locateFail(lambdav,lambda);
    ...
    {
        for (int ell=0; ell<_lambdagrid->numWavelengths(); ell++)
        {
...
{
    int p = NR::locateClip(_Tv, T);
    return NR::interpolateLinLin(T, _Tv[p], _Tv[p+1], _planckabsvv(c,p), _planckabsvv(c,p+1));
}
...
{
    int p = NR::locateClip(_planckabsvv[c], planckabs);
    return NR::interpolateLinLin(planckabs, _planckabsvv(c,p), _planckabsvv(c,p+1), _Tv[p], _Tv[p+1]);
}
