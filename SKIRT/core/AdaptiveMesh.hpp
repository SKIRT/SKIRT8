...
public:
    ...
    /** The constructor reads the AMR data from the specified adaptive mesh file. The \em
        ...
        issued. */
    AdaptiveMesh(AdaptiveMeshFile* meshfile, const vector<int>& fieldIndices, const Box& extent, Log* log=0);
...
private:
    ...
    double _integratedDensity{0.};               // total over all h and m (0 if there is no density distribution)
    vector<double> _integratedDensityv;          // the previous split per component

    // node tree
    AdaptiveMeshNode* _root{nullptr};            // root node representing the complete domain
    vector<AdaptiveMeshNode*> _leafnodes;        // leaf nodes indexed on m
};
