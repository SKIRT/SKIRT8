...
Position Trust6Geometry::generatePosition() const
{
    ...
    if (X < MF1)
    {
        R = RMAX1 * sqrt(random()->uniform());
        z = ZMIN1 + random()->uniform()*(ZMAX1-ZMIN1);
    }
    else if (X < MF1+MF2)
    {
        R = RMAX2 * sqrt(random()->uniform());
        z = ZMIN2 + random()->uniform()*(ZMAX2-ZMIN2);
    }
    else
    {
        R = RMAX3 * sqrt(random()->uniform());
        z = ZMIN3 + random()->gauss()*SIGMA3;
    }
