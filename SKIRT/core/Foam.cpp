...
{
    log->info("Growing foam of up to " + std::to_string(numcells) + " cells...");
    ...
    for (int pass = 0; ; pass++)
    {
        ...
        catch (FatalError& error)
        {
            ...
            for (string line : error.message()) log->warning(line);
...
void Foam::InitVertices()
{
    ...
    case 0:
        ...
        m_vMax= static_cast<int>(pow(2.0, m_nDim))+nDivi;
        if(m_OptVert==1)  m_vMax= static_cast<int>(pow(2.0, m_nDim));
        ...
        NoVert = static_cast<long>(pow(2.0,m_nDim));
        iVe=0;
        partition.Reset();
        do {
...
void Foam::InitCells()
{
    ...
    else if(m_nDim>0)
    {
        ...
        case 0:
            ...
            npow = static_cast<long>(pow(static_cast<double>(m_nDim),m_nDim));
            Perm  = new int[m_nDim];
...
int Foam::CellFill(int Status, FoamCell* Parent, int* Vertices, FoamVector* Posi, FoamVector* Size)
{
...
void Foam::Explore(FoamCell *Cell)
{
    ...
    double Factorial;
    double Wt, Dx, Dxx, Vsum, xBest=0., yBest=0.;
...
ee05:
    ...
    m_nEffev += static_cast<long>(NevEff);
    NevMC = CeSum[2];
...
void Foam::Varedu(double CeSum[5], int& kBest, double& xBest, double& yBest)
{
    ...
    for(int kProj=0; kProj<m_nProj; kProj++)
        if( m_MaskDiv[kProj])
        {
            ...
            int iLo = static_cast<int>(m_nBin*xMin);
            int iUp = static_cast<int>(m_nBin*xMax);
...
void Foam::Carver(int &kBest, double &xBest, double &yBest)
{
    // The case of the optimization of the maximum weight.
    // Determine the best edge-candidate for future cell division,
    // using results of the MC exploration run within the cell stored in m_HistEdg
    ...
    for(kProj=0; kProj<m_nProj; kProj++)
        if( m_MaskDiv[kProj] )
        {
            ...
            if( CarvTot > CarvMax)
            {
                ...
                xBest = (static_cast<double>(jLow))/m_nBin;
                yBest = (static_cast<double>(jUp+1))/m_nBin;
...
long Foam::PeekMax()
{
    ...
    if (iCell == -1) throw FATALERROR("PeekMax not found for cell no " + std::to_string(iCell));
    return(iCell);
}

///////////////////////////////////////////////////////////////////////////////

FoamCell* Foam::PeekRan()
{
...
void Foam::Divide(FoamCell *Cell)
{
...
void Foam::GenerCell(FoamCell*& pCell)
{
    //  Generation phase.
...
void Foam::GenerCel2(FoamCell*& pCell)
{
    // Generation phase
    // Return randomly chosen active cell with probability equal to its
    ...
    while(lo+1<hi)
    {
        hit = lo + static_cast<int>((hi-lo)*(Random-flo)/(fhi-flo)+0.5);
        if (hit<=lo)
            hit = lo+1;
        else if(hit>=hi)
            hit = hi-1;
...
void Foam::GetMCvect(double* MCvect)
{
...
double Foam::MCgenerate(double* MCvect)
{
...
void Foam::CheckAll()
{
    ...
    for(int iCell=1; iCell<=m_LastCe; iCell++)  // without root
    {
        ...
        if( ((Cell->GetDau0()==NULL) && (Cell->GetDau1()!=NULL) ) ||
            ...
            throw FATALERROR("Cell no " + std::to_string(iCell) + " has only one daughter");
        if( (Cell->GetDau0()==NULL) && (Cell->GetDau1()==NULL) && (Cell->GetStat()==0) )
            throw FATALERROR("Cell no " + std::to_string(iCell) + " has no daughter and is inactive");
        if( (Cell->GetDau0()!=NULL) && (Cell->GetDau1()!=NULL) && (Cell->GetStat()==1) )
            throw FATALERROR("Cell no " + std::to_string(iCell) + " has two daughters and is active");

        // checking parents
        if( (Cell->GetPare())!=m_Cells[0] )   // not child of the root
        {
            if ( (Cell != Cell->GetPare()->GetDau0()) && (Cell != Cell->GetPare()->GetDau1()) )
                throw FATALERROR("Cell no " + std::to_string(iCell) + " parent not pointing to this cell");
        }
        ...
        if(Cell->GetDau0()!=NULL)
        {
            if(Cell != (Cell->GetDau0())->GetPare())
                throw FATALERROR("Cell no " + std::to_string(iCell) + " daughter 0 not pointing to this cell");
        }
        if(Cell->GetDau1()!=NULL)
        {
            if(Cell != (Cell->GetDau1())->GetPare())
                throw FATALERROR("Cell no " + std::to_string(iCell) + " daughter 1 not pointing to this cell");
        }
    }
    ...
    if(m_nDim>0)
    {
        for(int iVert=0; iVert<=m_LastVe; iVert++)
        {
            ...
            if (Ref==0) throw FATALERROR("Vertex no " + std::to_string(iVert) + " NOT referenced");
        }
    }
    ...
    for(int iCell=0; iCell<=m_LastCe; iCell++)
    {
        ...
        if( (Cell->GetStat()==1) && (Cell->GetDriv()==0) )
            throw FATALERROR("Cell no " + std::to_string(iCell) + " is active but empty");
    }
}

///////////////////////////////////////////////////////////////////////////////////////

void Foam::RandomArray(int size, double* vect)
{
