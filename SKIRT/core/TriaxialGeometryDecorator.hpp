...
/** The TriaxialGeometryDecorator class is a geometry decorator that constructs a triaxial geometry
    based on a spherical (isotropic) geometry. The properties of an TriaxialGeometryDecorator
    object are a reference to the SpheGeometry object being decorated and the flattening parameters
    \f$p\f$ and \f$q\f$. If the original spherical geometry is characterized by the density profile
    \f$ \rho_{\text{s}}(r) \f$, the new geometry has as density \f[ \rho(x,y,z) = \frac{1}{p\,q}\,
