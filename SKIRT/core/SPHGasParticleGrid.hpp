...
public:
    ...
    SPHGasParticleGrid(const vector<SPHGasParticle>& pv, int gridsize);
    ...
    const vector<const SPHGasParticle*>& particlesFor(Vec r) const;
    ...
    vector<const SPHGasParticle*> particlesFor(const Box& box) const;
...
private:
    ...
    vector<vector<const SPHGasParticle*>> _listv;  // the m*m*m lists of particles overlapping each grid cell
