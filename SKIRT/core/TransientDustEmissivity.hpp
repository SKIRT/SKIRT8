...
{
    ITEM_CONCRETE(TransientDustEmissivity, DustEmissivity,
                  "transient heating dust emissivity (with full non-LTE treatment)")
        ATTRIBUTE_ALLOWED_IF(TransientDustEmissivity, "MultiGrainDustMix")
    ITEM_END()

    //============= Construction - Setup - Destruction =============

public:
    /** The destructor destroys the data structures created during setup. */
    ~TransientDustEmissivity();

    /** This function verifies that all dust components in the dust system have a dust mix based on
        the MultiGrainDustMix class and precalculates a bunch of values used in the dust emissivity
        computations. */
...
public:
    ...
    Array emissivity(const DustMix* mix, const Array& Jv) const override;
...
private:
    ...
    vector<const TDE_Grid*> _grids;

    // setupSelfBefore adds a calculator to these maps for each dust population in the simulation
    std::map<std::pair<const DustMix*,int>, const TDE_Calculator*> _calculatorsA;     // coarse grid
    std::map<std::pair<const DustMix*,int>, const TDE_Calculator*> _calculatorsB;     // medium grid
    std::map<std::pair<const DustMix*,int>, const TDE_Calculator*> _calculatorsC;     // fine grid
};
