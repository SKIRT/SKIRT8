...
public:
    ...
    TDE_Grid(const WavelengthGrid* lambdagrid, double Tmin, double Tmax, int NT, double ratio)
        : _hc(Constants::h()*Constants::c()), _hc2(2*_hc*Constants::c()), _k(Constants::k()),
...
void TransientDustEmissivity::setupSelfBefore()
{
    ...
    for (int h=0; h<Ncomp; h++)
    {
        ...
        const TDE_Grid* gridB = new TDE_Grid(lambdagrid, 2., Tupper, static_cast<int>(Tupper/widthB), ratioB);
        const TDE_Grid* gridC = new TDE_Grid(lambdagrid, 2., Tupper, static_cast<int>(Tupper/widthC), ratioC);
        ...
        {
            _calculatorsA.emplace(std::make_pair(mix,c), new TDE_Calculator(gridA, mix, c));
            _calculatorsB.emplace(std::make_pair(mix,c), new TDE_Calculator(gridB, mix, c));
            _calculatorsC.emplace(std::make_pair(mix,c), new TDE_Calculator(gridC, mix, c));
        }
    }
}
...
Array TransientDustEmissivity::emissivity(const DustMix* mix, const Array& Jv) const
{
    ...
    for (int c=0; c<Npop; c++)
    {
        ...
        const TDE_Calculator* calculatorA = _calculatorsA.at(std::make_pair(mix,c));
        ...
        string gcname = mgmix->grainCompositionName(c);
        ...
        if (!eqMass.count(gcname) || meanmass < eqMass.at(gcname))
        {
            ...
            if (Tmax-Tmin > deltaTeq && Teq < Tmax)
            {
                ...
                const TDE_Calculator* calculator = (Tmax-Tmin > deltaTmedium)
                                                   ? _calculatorsB.at(std::make_pair(mix,c))
                                                   : _calculatorsC.at(std::make_pair(mix,c));
