...
AdaptiveMesh::AdaptiveMesh(AdaptiveMeshFile* meshfile, const vector<int>& fieldIndices, const Box& extent, Log* log)
    : _log(log)
{
    // open the data file
    meshfile->open();
...
int AdaptiveMesh::numCells() const
{
    return _Ncells;
}
    ...
    while (node)
    {
        ...
        if (node==oldnode)
        {
            ...
            if (_log) _log->warning("Photon package seems stuck in dust cell "
                                    + std::to_string(node->cellIndex()) + " -- escaping");
