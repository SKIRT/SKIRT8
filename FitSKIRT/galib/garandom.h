...
inline int GARandomInt(int low, int high){
  double val=STA_CAST(double,high)-STA_CAST(double,low)+(double)1;
  val*=_GA_RND();
...
inline double GARandomFloat(){ return _GA_RND(); }
inline double GARandomFloat(double low, double high){
  double val=high-low; val*=_GA_RND(); return val+low;
}
...
inline GABoolean GAFlipCoin(double p){
  return((p == 1.0) ? gaTrue : (p == 0.0) ? gaFalse :
     ((GARandomFloat() <= p) ? gaTrue : gaFalse));
}

inline double GAGaussianFloat(double dev){ return (double)GAUnitGaussian()*dev; }
